...
def parameters():
	...
	parser.add_option("-o","--output-file",action="store",type="string",dest="filename",default="metadata.csv",help="Name of output CSV file")
...
def extract_information(saved_pages, extract=None):
	...
	if len(html_files) == saved_pages or extract:
		...
		fieldnames= ['court_name','registry_mark','decision_date','web_path','local_path', 'form_decision','decision_result','case_number']
...
def walk_pages(count_of_pages, case_type):
	...
	for i in t: # walk pages
		...
		if b_screens:
			session.capture_to(join(screens_dir_path,"find_screen_"+case_type+"_0"+str(i)+".png"),None,selector="#pagingBox0")
